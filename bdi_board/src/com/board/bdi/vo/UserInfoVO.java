package com.board.bdi.vo;

import java.io.Serializable;
import java.util.HashSet;
import java.util.Set;

/**
 * 새 테이블 모델 클래스.
 * 
 * @author generated by ERMaster
 * @version $Id$
 */
public class UserInfoVO implements Serializable {

	/** serialVersionUID. */
	private static final long serialVersionUID = 1L;

	/** 유저식별번호. */
	private Integer uinum;

	/** 유저정보아이디. */
	private String uiid;

	/** 유저정보비밀번호. */
	private String uipwd;

	/** 유저정보이름. */
	private String uiname;

	/** 유저정보이메일. */
	private String uiemeil;

	/** 게시판 목록. */
	private Set<BoardInfoVO> boardInfoSet;

	/** 댓글정보 목록. */
	private Set<CommentInfoVO> commentInfoSet;

	/**
	 * 생성자.
	 */
	public UserInfoVO() {
		this.boardInfoSet = new HashSet<BoardInfoVO>();
		this.commentInfoSet = new HashSet<CommentInfoVO>();
	}

	/**
	 * 유저식별번호을 설정합니다..
	 * 
	 * @param uinum
	 *            유저식별번호
	 */
	public void setUinum(Integer uinum) {
		this.uinum = uinum;
	}

	/**
	 * 유저식별번호을 가져옵니다..
	 * 
	 * @return 유저식별번호
	 */
	public Integer getUinum() {
		return this.uinum;
	}

	/**
	 * 유저정보아이디을 설정합니다..
	 * 
	 * @param uiid
	 *            유저정보아이디
	 */
	public void setUiid(String uiid) {
		this.uiid = uiid;
	}

	/**
	 * 유저정보아이디을 가져옵니다..
	 * 
	 * @return 유저정보아이디
	 */
	public String getUiid() {
		return this.uiid;
	}

	/**
	 * 유저정보비밀번호을 설정합니다..
	 * 
	 * @param uipwd
	 *            유저정보비밀번호
	 */
	public void setUipwd(String uipwd) {
		this.uipwd = uipwd;
	}

	/**
	 * 유저정보비밀번호을 가져옵니다..
	 * 
	 * @return 유저정보비밀번호
	 */
	public String getUipwd() {
		return this.uipwd;
	}

	/**
	 * 유저정보이름을 설정합니다..
	 * 
	 * @param uiname
	 *            유저정보이름
	 */
	public void setUiname(String uiname) {
		this.uiname = uiname;
	}

	/**
	 * 유저정보이름을 가져옵니다..
	 * 
	 * @return 유저정보이름
	 */
	public String getUiname() {
		return this.uiname;
	}

	/**
	 * 유저정보이메일을 설정합니다..
	 * 
	 * @param uiemeil
	 *            유저정보이메일
	 */
	public void setUiemeil(String uiemeil) {
		this.uiemeil = uiemeil;
	}

	/**
	 * 유저정보이메일을 가져옵니다..
	 * 
	 * @return 유저정보이메일
	 */
	public String getUiemeil() {
		return this.uiemeil;
	}

	/**
	 * 게시판 목록을 설정합니다..
	 * 
	 * @param boardInfoSet
	 *            게시판 목록
	 */
	public void setBoardInfoSet(Set<BoardInfoVO> boardInfoSet) {
		this.boardInfoSet = boardInfoSet;
	}

	/**
	 * 게시판를 추가합니다..
	 * 
	 * @param boardInfo
	 *            게시판
	 */
	public void addBoardInfo(BoardInfoVO boardInfo) {
		this.boardInfoSet.add(boardInfo);
	}

	/**
	 * 게시판 목록을 가져옵니다..
	 * 
	 * @return 게시판 목록
	 */
	public Set<BoardInfoVO> getBoardInfoSet() {
		return this.boardInfoSet;
	}

	/**
	 * 댓글정보 목록을 설정합니다..
	 * 
	 * @param commentInfoSet
	 *            댓글정보 목록
	 */
	public void setCommentInfoSet(Set<CommentInfoVO> commentInfoSet) {
		this.commentInfoSet = commentInfoSet;
	}

	/**
	 * 댓글정보를 추가합니다..
	 * 
	 * @param commentInfo
	 *            댓글정보
	 */
	public void addCommentInfo(CommentInfoVO commentInfo) {
		this.commentInfoSet.add(commentInfo);
	}

	/**
	 * 댓글정보 목록을 가져옵니다..
	 * 
	 * @return 댓글정보 목록
	 */
	public Set<CommentInfoVO> getCommentInfoSet() {
		return this.commentInfoSet;
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((uinum == null) ? 0 : uinum.hashCode());
		return result;
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public boolean equals(Object obj) {
		if (this == obj) {
			return true;
		}
		if (obj == null) {
			return false;
		}
		if (getClass() != obj.getClass()) {
			return false;
		}
		UserInfoVO other = (UserInfoVO) obj;
		if (uinum == null) {
			if (other.uinum != null) {
				return false;
			}
		} else if (!uinum.equals(other.uinum)) {
			return false;
		}
		return true;
	}

	@Override
	public String toString() {
		return "UserInfoVO [uinum=" + uinum + ", uiid=" + uiid + ", uipwd=" + uipwd + ", uiname=" + uiname
				+ ", uiemeil=" + uiemeil + ", boardInfoSet=" + boardInfoSet + ", commentInfoSet=" + commentInfoSet
				+ "]";
	}

}
